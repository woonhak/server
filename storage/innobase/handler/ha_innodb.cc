	...
	bool signal)
{
	...
	if (wsrep_thd_query_state(thd) == QUERY_EXITING) {
		WSREP_DEBUG("Victim query state QUERY_EXITING trx: " TRX_ID_FMT
			    " thread: %lu",
			    victim_trx->id,
			    thd_get_thread_id(thd));
		wsrep_thd_UNLOCK(thd);
	...
	if (wsrep_thd_exec_mode(thd) != LOCAL_STATE) {
		WSREP_DEBUG("Victim withdraw of non local for BF trx: "
			    TRX_ID_FMT ", thread: %lu exec_mode: %s",
			    victim_trx->id,
			    thd_get_thread_id(thd),
			    wsrep_thd_conflict_state_str(thd));
	}
	...
	case NO_CONFLICT:
		WSREP_DEBUG("Victim thread: %lu trx: " TRX_ID_FMT
			    " in NO_CONFLICT state",
			    thd_get_thread_id(thd),
			    victim_trx->id);
	...
	case QUERY_COMMITTING:
	{
		...
		if (wsrep_thd_exec_mode(thd) == REPL_RECV) {
			...
			switch (rcode) {
			case WSREP_WARNING:
				WSREP_DEBUG("Victim cancel commit warning thread:"
					    "%lu trx: " TRX_ID_FMT,
					    thd_get_thread_id(thd),
					    victim_trx->id);
				wsrep_thd_UNLOCK(thd);
			...
			case WSREP_OK:
				break;
			default:
				WSREP_ERROR("Victim cancel commit bad commit "
					    "exit thread: %lu trx: " TRX_ID_FMT
					    " rcode: %d ",
					    thd_get_thread_id(thd),
					    victim_trx->id,
					    rcode);
	...
	case QUERY_EXEC:
	{
		...
		if (wait_lock) {
			...
			WSREP_DEBUG("Kill query for victim thread: %lu trx: "
				    TRX_ID_FMT,
				    thd_get_thread_id(thd),
				    victim_trx->id);
	...
	case QUERY_IDLE:
	{
		WSREP_DEBUG("Victim kill trx QUERY_IDLE state thread: %lu "
			    "trx: " TRX_ID_FMT,
			    thd_get_thread_id(thd),
			    victim_trx->id);
		...
		if (wsrep_aborting_thd_contains(thd)) {
			WSREP_WARN("Victim is duplicate thd aborter thread: %lu "
				   "trx: " TRX_ID_FMT,
				   thd_get_thread_id(thd),
				   victim_trx->id);
		} else {
			wsrep_aborting_thd_enqueue(thd);
			WSREP_DEBUG("Enqueuing victim thread: %lu trx: " TRX_ID_FMT
				    " for abort",
				    thd_get_thread_id(thd),
				    victim_trx->id);
		}
	...
	default:
	{
		WSREP_WARN("Victim thread: %lu trx: " TRX_ID_FMT
			   " in bad wsrep query state: %s",
			   thd_get_thread_id(thd),
			   victim_trx->id,
			   wsrep_thd_query_state_str(thd));
		wsrep_thd_UNLOCK(thd);
