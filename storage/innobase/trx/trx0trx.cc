...
static std::atomic<int> n_pending_log_write;
...
static void background_log_write(void *)
{
  ...
  do
  {
    ...
    expected=1;
  } while (!n_pending_log_write.compare_exchange_strong(expected,0));
}

static tpool::task_group bck_write_group(1);
static tpool::waitable_task background_log_write_task(background_log_write, 0,
                                             &bck_write_group);
