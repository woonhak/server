                           ...
                           bool rm_mysql_schema)
{
  ...
  */
  ...
  query_cache_invalidate1(thd, path->str);

  if (!rm_mysql_schema)
  {
    tmp_disable_binlog(thd);
    (void) sp_drop_db_routines(thd, path->str); /* @todo Do not ignore errors */
#ifdef HAVE_EVENT_SCHEDULER
    Events::drop_schema_events(thd, path->str);
#endif
    reenable_binlog(thd);
  }
                     ...
                     bool silent)
{
  ...
  */
                                ...
                                true, false, true, false)))
  {
    ...
    drop_database_objects(thd, &rm_db, &cpath, rm_mysql_schema);
...
update_binlog:
  ...
  else if (mysql_bin_log.is_open() && !silent)
  {
    ...
    DBUG_ASSERT(query_pos != query_data_start);
    {
      int res;
      /*
        These DDL methods and logging are protected with the exclusive
