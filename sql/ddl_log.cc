...
static bool ddl_log_to_binary_log(THD *thd, DDL_LOG_ENTRY *ddl_log_entry,
                                  String *query, size_t init_query_length)
{
  if (mysql_bin_log.is_open())
  {
    if (!ddl_log_entry->next_entry)
    {
      /* Last entry, log all found tables and views */
    ...
    else if (query->length() + end_comment.length + NAME_LEN >
             thd->variables.max_allowed_packet)
    {
      /* Query is too big. Log it and disable all old entries */
      ddl_log_to_binary_log(thd, query);
      /* Remove already logged tables */
      query->length(init_query_length);
      return 1;
    }
  }
  return 0;
  ...
  */
  ...
  case DDL_LOG_DROP_TABLE_ACTION:
  {
    ...
    case DDL_DROP_PHASE_COLLECT:
      ...
      if (ddl_log_to_binary_log(thd, ddl_log_entry, &recovery_state.drop_table,
                                recovery_state.drop_table_init_length))
      {
        if (ddl_log_increment_phase_no_lock(entry_pos))
          break;
    ...
    case DDL_DROP_PHASE_RESET:
      /* We have already logged all previous drop tables. Clear the query */
  ...
  case DDL_LOG_DROP_VIEW_ACTION:
  {
    ...
    if (ddl_log_entry->phase == 0)
    {
      ...
      if (ddl_log_to_binary_log(thd, ddl_log_entry, &recovery_state.drop_view,
                                recovery_state.drop_view_init_length))
      {
        if (ddl_log_increment_phase_no_lock(entry_pos))
          break;
    ...
    {
      /* We have already logged all previous drop views. Clear the query */
  ...
  case DDL_LOG_DROP_DB_ACTION:
  {
    ...
    case DDL_DROP_DB_PHASE_INIT:
      drop_database_objects(thd, &db, &path,
